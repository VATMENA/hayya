<script lang="ts">
    import {can, roles, user} from "$lib/perms";
        ...
        >
            ...
            <DropdownMenu.Root>
                <DropdownMenu.Trigger asChild let:builder>
                    <Button builders={[builder]} variant="outline" class="mr-1 flex items-center space-x-2">
                        {#if user() !== null && roles() !== null}
                            ...
                            <span class="hidden font-bold sm:inline-block text-[15px] lg:text-base">{user().name_full}</span>
                        {:else}
                ...
                <DropdownMenu.Content class="w-56">
                    ...
                    <DropdownMenu.Label
                            class="font-normal text-foreground/60">{user().vacc === null ? "No vACC" : user().vacc}</DropdownMenu.Label>
                    {#if roles() !== null}
                        {#each roles() as role}
