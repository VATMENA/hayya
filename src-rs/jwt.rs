...
pub fn get_keypair() -> Ed25519KeyPair {
    Ed25519KeyPair::from_bytes(
        &hex::decode(
            std::env::var("MENAHQ_API_ED25519_KEYPAIR")
                .expect("required env var MENAHQ_API_ED25519_KEYPAIR is missing"),
        )
...
pub fn generate_token(user: &User) -> String {
    let claims =
        Claims::with_custom_claims(JwtData { user: user.clone() }, Duration::from_days(180));
