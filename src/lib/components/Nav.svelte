<script lang="ts">
	...
	const vacc_pages: Page = {
		...
		manage: {
			...
			visible:
				can($page.data.roles, ["|vacc.details.edit"]) &&
				$page.data.user.vaccId == $page.data.vacc_id,
		},
	};
					...
					>
						{#if $page.data.user !== null && $page.data.roles !== null}
							<Avatar.Root class="mr-2 h-5 w-5">
								...
								<Avatar.Fallback
									>{initials($page.data.user.name)}</Avatar.Fallback
								>
							</Avatar.Root>
							<span
				...
				<DropdownMenu.Content class="w-56">
					...
					<DropdownMenu.Label class="font-normal text-foreground/60"
						>{$page.data.user.vaccId === null
							? "No vACC"
							: $page.data.user.vaccId}</DropdownMenu.Label
					>
