...
export const GET: RequestHandler = async ({url, cookies}) => {
    ...
    let initial_vatsim_resp = await fetch("https://api.vatsim.net/v2/orgs/division/MENA?limit=1", {
        headers: {
    ...
    let real_roster_resp = await fetch(`https://api.vatsim.net/v2/orgs/division/MENA?limit=${need_to_pull}`, {
        headers: {
            "X-API-Key": VATSIM_CORE_API_TOKEN
        }
    });
    ...
    for (let roster_user of roster) {
        ...
        let vacc = roster_user.subdivision_id === null ? null : (Object.keys(vaccs).includes(roster_user.subdivision_id) ? roster_user.subdivision_id : null);
    ...
    console.log(`[RosterUpdate] Roster update task finished\tcreated=${created} updated=${updated} skipped=${skipped} total=${total}/${created+updated+skipped}`);
