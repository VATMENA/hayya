<script lang="ts">
  ...
  $: description = data.isUsersSession
    ? `${data.session.name} - ${humanReadableDate(data.session.scheduledTime)} with ${data.session.mentor.name}`
    : `${data.session.name} - ${humanReadableDate(data.session.scheduledTime)} with ${data.session.student.name}`;
...
<div class="flex items-center justify-between">
  <h2 class="text-3xl font-bold tracking-tight">
    {data.session.name} with {data.isUsersSession
      ? data.session.mentor.name
...
<div class="flex flex-col gap-1">
  <div class="flex flex-row gap-4">
    ...
    <div class="text-sm text-muted-foreground">
      <b>{data.session.mentor.name}</b>
      is training
      <b>{data.session.student.name}</b>
      on
      <b>{data.session.name}</b>
    </div>
    <div class="text-sm text-muted-foreground">
                  ...
                  <Form.Field {form} name="scoresheetUrl" class="flex flex-col">
                    <Form.Control let:attrs>
                      ...
                      <Input
                        {...attrs}
                        type="url"
                        bind:value={$formData.scoresheetUrl} />
                    </Form.Control>
                    <Form.FieldErrors />
                  </Form.Field>
            ...
            <Dialog.Root bind:open={deleteOpen}>
              ...
              <Dialog.Content>
                <Dialog.Header>
                  ...
                  <Dialog.Description>
                    This will immediately remove the session. It will NOT add
                    back the request on the student's behalf. It will be as if
                    they had never requested training in the first place.
                  </Dialog.Description>
                </Dialog.Header>
                <Dialog.Footer>
                  <Button
                  ...
                  <Button on:click={deleteSession} variant="destructive">
                    Yes, I'm sure
                  </Button>
                </Dialog.Footer>
              </Dialog.Content>
