  ...
  <Dialog.Root bind:open={assignOpen}>
    ...
    <Dialog.Content>
      <Dialog.Header>
        ...
        <Dialog.Description>
          They'll be set as the assigned mentor for this request, and they
          should reach out to the user to work out a time with them to complete
          this session.
        </Dialog.Description>
      </Dialog.Header>
      <AssignForm
        ...
        onsubmit={() => {
          assignOpen = false;
          toast.success(
            "Instructor has been assigned to that request. Check the Assigned tab for details.",
          );
        }}
        action="?/assign"
  ...
  <Dialog.Root bind:open={selfassignOpen}>
    ...
    <Dialog.Content>
      <Dialog.Header>
        ...
        <Dialog.Description>
          You'll be set as the assigned mentor for this request, and you should
          reach out to the user to work out a time with them to complete this
          session.
        </Dialog.Description>
      </Dialog.Header>
      <AssignForm
        ...
        onsubmit={() => {
          ...
          toast.success(
            "You've been assigned to that request. Check the Assigned tab for details.",
          );
        }}
        forceCid={$page.data.user.id}
        action="?/selfAssign"
        requestId={request.id} />
    </Dialog.Content>
  </Dialog.Root>
  ...
  <Dialog.Root bind:open={deleteOpen}>
    ...
    <Dialog.Content>
      <Dialog.Header>
        ...
        <Dialog.Description>
          This will remove the user's training request <b>permanently</b>
          .
        </Dialog.Description>
      </Dialog.Header>
      <Dialog.Footer>
        ...
        <Button
          ...
          type="submit"
          class="bg-red-600 text-red-950 hover:bg-red-700">
          Delete
        </Button>
      </Dialog.Footer>
    </Dialog.Content>
