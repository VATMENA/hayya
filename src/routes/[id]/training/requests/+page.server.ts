...
import { DELETE_REQUEST, SELF_ASSIGN_TO_REQUEST, TRAIN } from "$lib/perms/permissions";
...
export const actions: Actions = {
  ...
  selfAssign: async (event) => {
    ...
    if (!existingModel) {
      redirect(307, `/${event.params.id}/training`, {type: 'error', message: 'That request doesn\'t exist.'}, event.cookies);
    }
